...
for ($i=1;$i<=$_SESSION['CONTADOR'];$i++){
	for ($j=0; $j <$k ; $j++) { 
		if ($BYB_FIELD_ID[$j]==$_SESSION['FORMULARIO'][$i][2]) {
			...
			if ($_SESSION['FORMULARIO'][$i][2]==2) {$COMP_ISS_NUM =1;}//CONTIENE UN NUMERO
			if ($_SESSION['FORMULARIO'][$i][2]==3) {$COMP_DAT_JS =1;}//CONTIENE UNA FECHA
			...
			$BYB_FIELD_COD[$j] = str_replace("{title_sistem}",$_SESSION['TITLE_SISTEM'],$BYB_FIELD_COD[$j]);
